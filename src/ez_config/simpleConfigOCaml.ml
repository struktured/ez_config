...
let parse_config_file str =
  ...
  and parse_list end_kwd values =
    match Stream.peek str with
    | None -> Printf.ksprintf failwith "reached end of file before %s" end_kwd
    | Some (Kwd (";" | "," | ".")) ->
        Stream.junk str;
...
let parse filename ic =
  ...
  let list =
    try parse_config_file stream
    with e ->
      raise
        (LoadError (filename, ParseError (Stream.count s, Printexc.to_string e)))
  in
  Hashtbl.clear once_values;
  list
...
let comment option_comment =
  ...
  List.iter
    ...
    lines;
  let max_length = if !max_length > 74 then 74 else !max_length in
  let spaces = String.make max_length ' ' in
  let lines =
    List.map
      (fun line ->
        let len = String.length line in
        if len < max_length then line ^ String.sub spaces 0 (max_length - len)
        else line)
      lines
  in
